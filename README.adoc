...
Place [hotspot=features file=0]`RUN features.sh` between the `COPY` commands in the `Containerfile`. The `features.sh` script adds the Liberty features that your application is required to operate.
