...
efficiently with containers. You can run a container from a container image, which can be defined by a Containerfile or a Dockerfile.
...
Podman and https://buildah.io[Buildah^] are related open-source container tools built to run on most Linux platforms.
Buildah is designed specifically for building container images from either a Containerfile or the command line. In contrast, Podman further incorporates functionality to maintain those images, 
in addition to creating and running containers. Podman uses Buildah's functionality to create the container image it uses.
...
instructions in Containerfiles to create a containerized version of the application.

The syntax for a Containerfile and a Dockerfile are the same. Podman can build your container image using either Containerfiles or Dockerfiles. Containerfiles will be used in this guide.

A `Containerfile` is a collection of instructions for building a container image that can then be run as a container. These files can be interpreted directly by Buildah, or via Podman.
The `podman build` command uses Buildah to build your container image. As each instruction is run in a `Containerfile`, a new image layer is created.
These layers, which are known as intermediate images, are created when a change is made to your container image.

Every `Containerfile` begins with a parent or base image over which various commands are run. For example, you can start your image from scratch and run commands that download and install a Java runtime, or you can start from an image that already contains a Java installation.

Learn more about Podman on the https://podman.io/getting-started/[official Podman page^].

=== Creating your Containerfiles
You will be creating two container images to run the `inventory` service and `system` service. The first step is to create Containerfiles for both services.

[role="code_command hotspot file=0", subs="quotes"]
----
#Create the `Containerfile` for the inventory service.#
...
In this guide, you're using the production image from the IBM Container Registry (ICR),
`icr.io/appcafe/open-liberty:full-java11-openj9-ubi`, as your parent image. This image is tagged with the word `full`, meaning it includes all Liberty features.
Because this significantly expands the image size for some features that will likely go unused, these images are recommended for development only.

In production, to minimize your image footprint, it is recommended that you use the `kernel-slim` image from `icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi`.
This will install the basic server, to which you can then add all necessary features via the usage pattern detailed in the Open Liberty https://github.com/OpenLiberty/ci.docker#building-an-application-image[container image documentation^].
...
You can find links to all official images and Universal Base Images (ubi) on the Open Liberty https://github.com/OpenLiberty/ci.docker#container-images[container image repository^].

It is also recommended to label your container images with the [hotspot=label file=0]`LABEL` command, as the label information can help you manage your images.
...
including `icr.io/appcafe/open-liberty:full-java11-openj9-ubi` used in this case, run by default with the `USER 1001` (non-root) user for security purposes. Otherwise, the files and directories that are copied over are owned by the root user.

Place the [hotspot=configure-sh file=0]`RUN configure.sh` command at the end to get a pre-warmed container image. It improves the startup time of running your container.

The `Containerfile` for the `system` service follows the same instructions as the `inventory` service, except that some [hotspot=name hotspot=summary file=1]`labels` are updated, and the [hotspot=copy-war file=1]`system.war` archive is copied into `/config/apps`.

[role="code_command hotspot file=1", subs="quotes"]
----
#Create the `Containerfile` for the system service.#
...
Now that your microservices are packaged and you have written your Containerfiles, you will build your container images by using the `podman build` command.
...
The `-t` flag in the `podman build` command allows the container image to be labeled (tagged) in the `name[:tag]` format. 
...
Your two images, `inventory` and `system`, should appear in the list of all container images:
...
Now that you have your two images built, you will run your microservices in containers:
...
container logs for any potential problems. Run the `podman stats system` and `podman stats inventory` commands to display a live stream of usage statistics for your containers. You can also double-check that your Containerfiles are correct. When you
...
Next, retrieve the `system` container's IP address by using the `system` container's name that is defined when it ran the containers. 
...
In this case, the IP address for the `system` service is `10.88.0.2`. Take note of this IP address to add the system properties to the `inventory` service. 
...
You have just built container images and run two microservices on Open Liberty in containers. 
