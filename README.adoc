...
Once your `Podman` containers are running, run the following command to see the list of required features installed by `features.sh`:
[role='command']
```
docker exec -it /opt/ol/wlp/bin/productInfo featureInfo
```
