...
:page-releasedate: 2022, 2023-03-07
...
In this guide, you're using an official image from the IBM Container Registry (ICR), `icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi`, as your parent image. This image is tagged with the word `full`, meaning it includes all Liberty features.
...
including `icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi` used in this case, run by default with the `USER 1001` (non-root) user for security purposes. Otherwise, the files and directories that are copied over are owned by the root user and a non-root user will be unable to access them.
