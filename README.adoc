...
Replace the parent image with [hotspot=kernel-slim file=0]`icr.io/appcafe/open-liberty:kernel-slim-java11-openj9-ubi` at the top of your Containerfile. This image contains the `kernel-slim` tag that's recommended when deploying to production.
...
Verify that the images have built by executing the following command to list all the local images:
...
Once your containers are running, run the following command to see the list of required features installed by `features.sh`:
[role='command']
```
podman exec -it /opt/ol/wlp/bin/productInfo featureInfo
```
