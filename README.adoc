...
Podman and https://buildah.io[Buildah^] are related open-source container tools built to run on most Linux platforms. While Buildah is designed specifically
for building container images, Podman further incorporates functionality to maintain those images, in addition to the creation and running of containers.
Podman uses Buildah's functionality to create the container image it uses.
...
A `Containerfile` is a collection of instructions for building a container image that can then be run as a container.
