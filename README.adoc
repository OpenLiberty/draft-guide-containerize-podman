...
This project is a multi-module Maven project that is made up of the `system` and inven`tory microservices. Each microservice is located in its own corresponding directory, `system` and `inventory`.
...
The syntax for a `Containerfile` and a Dockerfile are the same. 
...
The `podman build` command uses Buildah to build your container image. As each instruction runs in a `Containerfile` file, a new image layer is created.
These layers, which are known as intermediate images, are created when a change is made to your container image.

Every `Containerfile` file begins with a parent or base image over which various commands are run. For example, you can start your image from scratch and run commands that download and install a Java runtime, or you can start from an image that already contains a Java installation.
